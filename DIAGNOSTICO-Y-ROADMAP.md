...
| Backend         | Fastify en Cloud Run, revisión `fauzet-api-00009-div` con 100% del tráfico        | Google Auth y rutas Mercado Pago TEST desplegados detrás de sus controles; health/readiness y proxy verificados                                     | Monitoreo, alertas, métricas operativas y siguientes integraciones                                                                    |
| Datos           | PostgreSQL en Cloud SQL, ledger y 23 migraciones aplicadas                        | Identidad Google persistente y hardening de pagos, órdenes, intentos, webhook inbox, inventario y reembolsos                                        | Outbox, alertas y reportes operativos                                                                                                 |
...
| Auth            | Registro, login, Google, verificación, recuperación y sesiones persistentes       | Google/Firebase publicado con vinculación segura; UI de seguridad/perfil ampliada                                                                   | Primera prueba humana de Google; TOTP 2FA, alertas y recuperación reforzada                                                           |
...
- Google Auth/Firebase está publicado y verificado técnicamente; falta una primera prueba humana del popup. KYC, custodia y demás terceros siguen pendientes o en proceso.
...
5. [x] Migración 23, API y web publicadas; botón visible/habilitado y proxy verificados en `fauzet.app`.
6. [ ] Propietario: ejecutar una primera prueba real de acceso con Google y confirmar que entra al dashboard.
...
- [x] Google Auth con Firebase y vinculación segura de cuentas publicado, cubierto por pruebas y validado técnicamente; prueba humana del popup pendiente.
...
| Now                                                                                                                                             | Next                                                                                                                                                    | Later                                                                                                                                                                    |
| ----------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Google Auth publicado; API `00009-div`, migración 23 y web verificadas; Mercado Pago TEST fail-closed; scheduler fiat pausado y gates apagados. | Propietario prueba Google; desarrollo adapta Mercado Pago de Checkout Pro a Checkout API y ejecuta prueba TEST allowlisted; luego recibos y activación. | Integrar Stripe como segundo proveedor; piloto real y cripto sólo tras gates legales, de custodia, reservas y seguridad; marketplace, apps y token público condicionado. |
